...
3.  **Data Transmission:** The application is served exclusively over **HTTPS**, encrypting all data in transit between the client and server.
4.  **Input Validation:** All user input (e.g., usernames, friend IDs) is validated on both the client and server-side to prevent malicious input and ensure data integrity.
5.  **Dependencies:** Application dependencies are regularly monitored for vulnerabilities using standard toolchains like `npm audit`.
